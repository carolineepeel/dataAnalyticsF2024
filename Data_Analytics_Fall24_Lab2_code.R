...
setwd("C:\\Users\\carol\\OneDrive\\Documents\\data_analytics\\lab02")
...
### Comparing distribtions of 3 variables
boxplot(FSH.new, FSS.new, FCD.new, names=c("FSH.new", "FSS.new", "FCD.new"))
...
qqplot(qnorm(ppoints(200)),FSH.new)
qqline(FSH.new)

qqplot(rnorm(1000),FSH.new)
qqline(FSH.new)
...
qqplot(qnorm(ppoints(200)),FSS.new)
qqline(FSS.new)
...
qqplot(qnorm(ppoints(200)),FCD.new)
qqline(FCD.new)

qqplot(rnorm(1000),FCD.new)
qqline(FCD.new)
...
plot(ecdf(FCD.new), do.points=FALSE) 

plot(ecdf(rnorm(1000, 45, 10)), do.points=FALSE, main="Norm Dist vs. FCD.new ECDF")
lines(ecdf(FCD.new))

plot(ecdf(FCD.old), do.points=FALSE, main="FCD.old vs. FCD.new ECDF")
lines(ecdf(FCD.new))

## Plot 2

plot(ecdf(FSS.new), do.points=FALSE) 

plot(ecdf(rnorm(1000, 45, 10)), do.points=FALSE, main="Norm Dist vs. FSS.new ECDF")
lines(ecdf(FSS.new))

plot(ecdf(FSS.old), do.points=FALSE, main="FSS.old vs. FSS.new ECDF")
lines(ecdf(FSS.new))

#### Populations Dataset ####

## read data
populations_2023 <- read.csv("C:\\Users\\carol\\OneDrive\\Documents\\data_analytics\\lab02\\countries_populations_2023.csv")
